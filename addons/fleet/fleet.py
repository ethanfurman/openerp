...
class fleet_vehicle_cost(osv.Model):
    ...
    _order = 'vehicle_id asc, date desc'
...
class fleet_vehicle(osv.Model):
    ...
    def _get_dmv_expiry_days(self, cr, uid, ids, field_names, unknown_none, context=None):
    ...
    _columns = {
        ...
        'color': fields.char('Lease #', size=32, help='???'),
        ...
        'car_value': fields.float('Value', help='Value of the bought vehicle'),
        'dmv_class': fields.char('DVM Class', size=10, help='Weight Class'),
        'fast_track_no': fields.char('Fast Track #', size=10, help='Fast Track #'),
...
class fleet_vehicle_odometer(osv.Model):
    ...
    def _vehicle_odometer_reading_source(self, cr, uid, ids, prop, unknown_none, context=None):
        result = {}
        records = self.browse(cr, uid, ids)
        for rec in records:
            source = False
            if rec.cost_ids:
                if rec.cost_ids[0].cost_subtype_id:
                    source = rec.cost_ids[0].cost_subtype_id.name
                else:
                    source = rec.cost_ids[0].cost_type.title()
            result[rec.id] = source
        return result
    ...
    _columns = {
        ...
        'cost_ids': fields.one2many('fleet.vehicle.cost', 'odometer_id', 'Source Record'),
        'cost_type': fields.function(_vehicle_odometer_reading_source, type="char", string="Type"),
    }
    _defaults = {
...
class fleet_vehicle_log_fuel(osv.Model):
    ...
    def _calc_fuel_rate(self, cr, uid, ids):
        prop = 'mpg'
        result = defaultdict(dict)
        records = self.browse(cr, uid, ids)
        vehicles = self.pool.get('fleet.vehicle.cost')
        for rec in records:
            result[rec.id][prop] = False
            history = vehicles.browse(cr, uid, vehicles.search(cr, uid, [
                ('vehicle_id.name','=',rec.cost_id.vehicle_id.name),
                ('cost_type','=','fuel'),
                ]))
            history.sort(key=lambda x: x.odometer, reverse=True)
            if rec.cost_id in history:
                index = history.index(rec.cost_id)
                history = history[index+1:index+2]
            if history:
                target = history[0]
            else:
                target = False
    ...
    def create(self, cr, uid, values, context=None):
        new_id = super(fleet_vehicle_log_fuel, self).create(cr, uid, values, context=context)
        self.write(cr, uid, [new_id], {}, context=context)
        return new_id

    def write(self, cr, uid, ids, values, context=None):
        result = self._calc_fuel_rate(cr, uid, ids)
        for id, d in result.items():
            if id in values:
                values[id].update(d)
            else:
                values.update(d)
        return super(fleet_vehicle_log_fuel, self).write(cr, uid, ids, values, context=context)
    ...
    _columns = {
        'liter': fields.float('Gallon'),
        'price_per_liter': fields.float('Price Per Gallon'),

...
class BaseModel(object):
    ...
    _sql_constraints = []
    _protected = [
        'read', 'write', 'create', 'default_get', 'perm_read', 'unlink', 'fields_get',
        'fields_view_get', 'search', 'name_get', 'distinct_field_get', 'name_search',
        'copy', 'import_data', 'search_count', 'exists',
        ]
    ...
    def create_instance(cls, pool, cr):
        ...
        if parent_names:
            ...
            for parent_name in ((type(parent_names)==list) and parent_names or [parent_names]):
                ...
                if not parent_model:
                    raise TypeError('The model "%s" specifies a non-existent parent class "%s"\n'
                        'You may need to add a dependency on the parent class\' module. (in `__openerp__.py`' % (name, parent_name))
        ...
        **Expressing a search domain (args)**
        ...
        for arg in args:
            ...
            if (
                    ...
                    not (isinstance(period, basestring) and hasattr(Period, period))
                    ):
                new_args.append(arg)
                continue
    ...
    def _m2m_raise_or_create_relation(self, cr, f):
        ...
        if not cr.dictfetchall():
            ...
            index1 = '%s_%s_index' % (m2m_tbl, col1)
            index2 = '%s_%s_index' % (m2m_tbl, col2)
            if len(index1) >= 64:
                raise except_orm('bad many2many field', 'dependent index name %r is too long (max 63 characters)' % index1)
            elif len(index2) >= 64:
                raise except_orm('bad many2many field', 'dependent index name %r is too long (max 63 characters)' % index2)
            cr.execute('CREATE INDEX "%s" ON "%s" ("%s")' % (index1, m2m_tbl, col1))
            cr.execute('CREATE INDEX "%s" ON "%s" ("%s")' % (index2, m2m_tbl, col2))
    ...
    def browse(self, cr, uid, select=[None], context=None, list_class=None, fields_process=None):
        ...
        if not isinstance(select, bool) and isinstance(select, (int, long)):
